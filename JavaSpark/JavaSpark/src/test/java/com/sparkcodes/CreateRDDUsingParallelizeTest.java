...
public class CreateRDDUsingParallelizeTest {
    private final SparkConf sparkConf = new SparkConf().setMaster("local[*]").setAppName("com.sparkcodes.CreateRDDUsingParallelizeTest");

    @Test
    @DisplayName("Create an empty RDD with no partitions in Spark")
    public void createAnEmptyRDDWithNoPartitionsInSpark() {
    ...
    public void createAnEmptyRDDWithDefaultPartitionsInSpark() {
        try (final var sparkContext = new JavaSparkContext(sparkConf)) {
    ...
    void createSparkRDDUsingParallelizeMethod() {
        try (final var sparkContext = new JavaSparkContext(sparkConf)) {
            final var data = Stream.iterate(1,i->i+1).limit(5).collect(Collectors.toList());
