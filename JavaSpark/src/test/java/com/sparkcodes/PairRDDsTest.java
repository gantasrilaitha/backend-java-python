...
public class PairRDDsTest {
    ...
    void testMapToPairInSparkRDD(final String testFilePath) {
        try (final var sparkContext = new JavaSparkContext(sparkConf)){
            ...
            final var pairRDD = myRdd.mapToPair(line->new Tuple2<>(line,line.length()));
    ...
    void testReduceByKeyInSparkRDD(final String testFilePath) {
        try (final var sparkContext = new JavaSparkContext(sparkConf)){
            ...
            counts.take(10).forEach(tuple->System.out.printf("Total strings of  length %d are %d%n",tuple._1,tuple._2));
            System.out.println("--------------------");
        }
    }
    ...
    void testDistinctInSparkRDD(final String testFilePath) {
        try (final var sparkContext = new JavaSparkContext(sparkConf)) {
            ...
            uniqueWordsPairs.take(5).forEach(tuple -> System.out.println(tuple._1));
